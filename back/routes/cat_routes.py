...
def get_cats():
    ...
    return jsonify(json.loads(json_util.dumps(cats)))
...
def generate_cats():
    ...
    try:
        ...
        return jsonify(json.loads(json_util.dumps(generated_cats))), 201
...
def get_random_cat():
    """Получить случайного кота с заданной редкостью"""
    try:
        # Получаем параметр редкости из запроса
        ...
        # Получаем общее количество котов с заданным фильтром
        count = collection.count_documents(filter_query)
        if count == 0:
            # Если нет котов с заданной редкостью, убираем фильтр
            if rarity:
                filter_query = {}
                count = collection.count_documents({})
            
            # Если все еще нет котов, возвращаем ошибку
            if count == 0:
                return {'error': 'Нет доступных котов'}, 404
        
        # Выбираем случайного кота из отфильтрованных
        random_index = random.randint(0, count - 1)
        random_cat = list(collection.find(filter_query).skip(random_index).limit(1))[0]
