...
def get_user(user_id):
    ...
    if not user:
        print(f"Создаем нового пользователя с ID: {user_id}")
        new_user = {
            'user_id': user_id,
            'nickname': f'User_{user_id[-6:]}',
            'username': f'user_{user_id[-6:]}',
            'coins_count': 100,  # Начальные монеты
            'claps_count': 50,   # Начальные хлопки
            'created_at': int(time.time())
        }
    ...
    if user:
        return jsonify({
            ...
            'claps_count': user.get('claps_count', 0)
        })
...
def update_user(user_id):
    ...
    try:
        print(f"Получен запрос на обновление пользователя {user_id}: {data}")
        ...
        if not user:
            ...
            new_user = {
                ...
                'nickname': data.get('nickname', f'User_{user_id[-6:]}'),
                'username': data.get('username', f'user_{user_id[-6:]}'),
                'coins_count': data.get('coins_count', 100),  # Начальные значения
                'claps_count': data.get('claps_count', 50),   # Начальные значения
                'friends': data.get('friends', []),
                'mood': data.get('mood', 'normal'),
                'lastInteractionTime': data.get('lastInteractionTime', int(time.time() * 1000)),
                'created_at': int(time.time())
            ...
            return jsonify({'success': True, 'message': 'User created', 'id': str(result.inserted_id)})
        ...
        # Особая обработка счетчиков для логирования
        if 'coins_count' in data:
            update_data['coins_count'] = data['coins_count']
            print(f"Обновление счетчика монет для {user_id}: {user.get('coins_count', 0)} -> {data['coins_count']}")
            
        if 'claps_count' in data:
            update_data['claps_count'] = data['claps_count']
            print(f"Обновление счетчика хлопков для {user_id}: {user.get('claps_count', 0)} -> {data['claps_count']}")
        
        # Добавляем остальные поля кроме зарезервированных
        for key, value in data.items():
            if key not in ['_id', 'user_id', 'coins_count', 'claps_count'] and key != 'friend':
        ...
        if update_data:
            print(f"Применяем обновления для пользователя {user_id}: {update_data}")
            users_collection.update_one({'user_id': user_id}, {'$set': update_data})
            
            # Для счетчиков записываем транзакцию
            if 'coins_count' in update_data or 'claps_count' in update_data:
                transaction = {
                    'user_id': user_id,
                    'type': 'update',
                    'coins_delta': update_data.get('coins_count', user.get('coins_count', 0)) - user.get('coins_count', 0) if 'coins_count' in update_data else 0,
                    'claps_delta': update_data.get('claps_count', user.get('claps_count', 0)) - user.get('claps_count', 0) if 'claps_count' in update_data else 0,
                    'timestamp': int(time.time())
                }
                transactions_collection.insert_one(transaction)
                print(f"Записана транзакция: {transaction}")
...
def get_top_users():
    ...
    sort_by = request.args.get('sort_by', 'coins')  # Новый параметр для сортировки
    ...
    sort_field = 'claps_count' if sort_by == 'claps' else 'coins_count'
    print(f"Сортировка топа по полю: {sort_field}")
    
    # Формируем запрос в зависимости от периода и поля сортировки
    pipeline = [
    ...
    return jsonify(json.loads(json_util.dumps(top_users)))
# Покупка валюты
...
def purchase_currency(user_id):
    ...
    try:
        ...
        # Возвращаем обновленного пользователя
        updated_user = users_collection.find_one({'user_id': user_id})
        return jsonify(json.loads(json_util.dumps(updated_user)))
    except Exception as e:
        return {'error': str(e)}, 500
...
# Добавить кота пользователю
@user_bp.route('/<user_id>/cats', methods=['POST'])
def add_cat_to_user(user_id):
    """Добавить кота пользователю"""
    data = request.json
    if not data:
        return {'error': 'Данные не предоставлены'}, 400
    ...
    try:
        ...
        # Создаем запись о коте пользователя
        cat_data = {
            'cat_id': data.get('cat_id'),
            'name': data.get('name', 'Безымянный кот'),
            'images': data.get('images', {}),
            'color': data.get('color', 'Классический'),
            'purchase_date': data.get('purchase_date', datetime.now().isoformat()),
            'added_at': int(time.time())
        }
        
        # Если у пользователя еще нет списка котов, создаем его
        if 'cats' not in user:
            users_collection.update_one(
                {'user_id': user_id},
                {'$set': {'cats': [cat_data]}}
            ...
            # Иначе добавляем кота в существующий список
            users_collection.update_one(
                {'user_id': user_id},
                {'$push': {'cats': cat_data}}
            )
        
        # Возвращаем добавленного кота
        return jsonify(json.loads(json_util.dumps(cat_data)))
    except Exception as e:
        return {'error': str(e)}, 500
      
      
# Установить выбранного кота
@user_bp.route('/<user_id>/selected-cat', methods=['PUT'])
def set_selected_cat(user_id):
    """Установить выбранного кота для пользователя"""
    data = request.json
    if not data or 'cat_id' not in data:
        return {'error': 'Необходимо указать ID кота'}, 400
    ...
    try:
        ...
        # Проверяем, есть ли такой кот у пользователя
        user_cats = user.get('cats', [])
        cat_exists = False
        ...
        if not cat_exists:
            return {'error': 'Указанный кот не принадлежит пользователю'}, 404
        
        # Устанавливаем выбранного кота
        users_collection.update_one(
            {'user_id': user_id},
            {'$set': {'selected_cat_id': cat_id}}
        )
      ...
      # Добавить этот метод в файл user_routes_v2.py

# Получить выбранных котов всех пользователей
@user_bp.route('/selected-cats', methods=['GET'])
def get_all_selected_cats():
    """Получить выбранных котов всех пользователей"""
    try:
        result = []
        # Находим всех пользователей с выбранными котами
        users = users_collection.find({'selected_cat_id': {'$exists': True}})
        
        for user in users:
            user_id = user.get('user_id')
            selected_cat_id = user.get('selected_cat_id')
            
            # Находим кота из списка котов пользователя
            selected_cat = None
            for cat in user.get('cats', []):
                if cat.get('cat_id') == selected_cat_id:
                    selected_cat = cat
                    break
        ...
        print(f"Ошибка при получении выбранных котов: {str(e)}")
        return {'error': str(e)}, 500